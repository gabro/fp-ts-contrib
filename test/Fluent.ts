...
import { getMonoid, getSetoid, none, option, Option, some, URI, getOrd, getShow } from 'fp-ts/lib/Option'
...
import { nonEmptyArray, fromNonEmptyArray } from 'fp-ts/lib/NonEmptyArray2v'
...
describe('fluent', () => {
  ...
  it('bimap', () => {
    ...
    assert.deepStrictEqual(fluent(right<string, number>(1)).bimap(len, double).value, right(2))
    assert.deepStrictEqual(fluent(left<string, number>('err')).bimap(len, double).value, left(3))
  })

  it('mapLeft', () => {
    const fluent = F.fluent(either)
    const len = (s: string): number => s.length
    assert.deepStrictEqual(fluent(right<string, number>(1)).mapLeft(len).value, right(1))
    assert.deepStrictEqual(fluent(left<string, number>('err')).mapLeft(len).value, left(3))
  })
  ...
  it('foldr', () => {
    assert.strictEqual(
      fluent(some('aaa')).foldr(1, (a, acc) => acc + a.length),
      option.foldr(some('aaa'), 1, (a, acc) => acc + a.length)
    )
  })
  ...
  it('foldrWithIndex', () => {
    const fluent = F.fluent(array)
    assert.strictEqual(
      fluent([1, 2, 3]).foldrWithIndex(1, (i, a, acc) => i + acc + a),
      array.foldrWithIndex([1, 2, 3], 1, (i, a, acc) => i + acc + a)
    )
  })
  ...
  it('separate', () => {
    ...
    assert.deepStrictEqual(
      fluent([right<string, number>(1), left<string, number>('err'), right<string, number>(3)]).separate(),
      { left: ['err'], right: [1, 3] }
    )
  })
  ...
  it('promap', () => {
    ...
    const x = fluent(new Reader((e: E) => e.count + 1)).promap((s: string) => ({ count: s.length }), n => n > 2).value
    assert.deepStrictEqual(x.run('aa'), true)
    assert.deepStrictEqual(x.run('a'), false)
  })
})
